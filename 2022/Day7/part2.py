...
class Node:
    ...
    def SumDirs(self, maxsize):
        ...
        return tot
    def GetMinDirToDel(self, needed_space):
        if self.CalcSize() >= needed_space:
            candidates = [child.GetMinDirToDel(needed_space) for child in self.children if child.type == "dir" and child.CalcSize() >= needed_space]
            candidates.append(self.CalcSize())
...
update_sz = 30000000
cur_space = root.CalcSize()
unused_space = fs_size_limit - cur_space
min_del_space = update_sz - unused_space
print(root.GetMinDirToDel(min_del_space))
