...
for cmd in range(16):
    possible[cmd] = list(enumerate(commands))
...
for i in input:
    ...
    if 'Before' in i:
        ...
        possible[instr[0]] = [(ix, func) for (ix, func) in possible[instr[0]] if func(before, instr) == after]
    else:
...
for _ in range(16):
    for c in range(16):
        if len(possible[c]) == 1:
            for c2 in range(16):
                if c2 != c:
                    possible[c2] = [(ix, func) for (ix, func) in possible[c2] if ix != possible[c][0][0]]

print(possible)
registers = [0, 0, 0, 0]
for l in input2.strip().split('\n'):
    instr = list(map(int, re.findall('-?\d+', l)))
    registers = possible[instr[0]][0][1](registers, instr)
print(registers[0])