...
def simulate(elfDmg):
    ...
    for y in range(len(input)):
        for x in range(len(input[y])):
            c = input[y][x]
            if c in "GE":
                units.append(Unit(c, x, y, 3 if c == 'G' else elfDmg))
                input[y][x] = '.'
    for y in range(len(input)):
        for x in range(len(input[y])):
            if input[y][x] == '.':
    ...
    while True:
        ...
        for ix, u in enumerate(orderedUnits):
            ...
            if atkList:
                ...
                targetedEnemy.attack(u.dmg)
                
                if targetedEnemy.type == 'E' and not targetedEnemy.alive:
                    return False, 0

                if (len(set(u.type for u in units if u.alive)) == 1):
                    if ix == len(orderedUnits) - 1:
                        nRounds += 1
                    return True, nRounds * sum(u.hp for u in units if u.alive)
                    
        nRounds += 1

elfDmg = 4
while True:
    done, score = simulate(elfDmg)
    if done:
        print(score)
        break
    elfDmg += 1
