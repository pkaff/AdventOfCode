    ...
    for y in range(0, len(input)):
        for x in range(0, len(input[y])):
            ...
            if input[nextY][nextX] == 'x':
                ...
                print(nextX, nextY)
                print(cartCount)
                input[nextY][nextX] = tracks[nextY][nextX]
            else:
                hasMoved[(nextX, nextY)] = True
    if cartCount == 1:
        for finalY in range(0, len(input)):
            for finalX in range(0, len(input[finalY])):
                curChar = input[finalY][finalX]
                if curChar == 'v' or curChar == '^' or curChar == '>' or curChar == '<':
                    print(finalX, finalY)
        sys.exit()
    #for i in input:
    #    print(''.join(i))