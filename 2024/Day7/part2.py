...
def greedy_search(target, cur_res, to_eval):
    ...
    if cur_res > target:
        return False
    return greedy_search(target, int(str(cur_res) + str(to_eval[0])), to_eval[1:]) or greedy_search(target, cur_res * to_eval[0], to_eval[1:]) or greedy_search(target, cur_res + to_eval[0], to_eval[1:])
